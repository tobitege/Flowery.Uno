...
DaisyInput wraps an inner `TextBox` and forwards common TextBox properties (`Text`, `PlaceholderText`, `MaxLength`, `AcceptsReturn`, `TextWrapping`, `IsReadOnly`).
...
| `StartIcon` | `Geometry?` | `null` | Icon displayed at the start (left) of the input. |
| `EndIcon` | `Geometry?` | `null` | Icon displayed at the end (right) of the input. |
| `BorderRingBrush` | `Brush?` | `null` | Custom brush for the focus ring (overrides default). |
| `IsReadOnly` | `bool` | `false` | When true, text cannot be edited but can be selected/copied. |
