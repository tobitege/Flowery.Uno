...
`DaisyNumericUpDown` is a numeric input control that extends WinUI's `NumberBox` with DaisyUI styling. It provides increment/decrement buttons (spin buttons) for adjusting numeric values, along with direct text entry support. The control follows the same visual style and variant system as `DaisyInput`.
...
- The control inherits all keyboard shortcuts from WinUI's NumberBox (Up/Down arrows, Page Up/Down)
